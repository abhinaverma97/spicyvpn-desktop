                  ...
                  <>
                    <div className="space-y-3">
                      ...
                      <div className="bg-black/50 border border-white/5 rounded-xl p-4">
                        ...
                        <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full transition-all duration-1000 ${usedPct > 80 || isOutOfData ? 'bg-red-500' : 'bg-emerald-500'}`} style={{ width: `${Math.min(100, usedPct)}%` }} />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-xs text-white/40 px-2">
                      ...
                      <div className="flex gap-4">
                        <button onClick={() => setShowLogs(true)} className="flex items-center gap-1 hover:text-white transition-colors"><ScrollText className="w-3 h-3" /> Logs</button>
                        <button onClick={() => setIsEditingLink(true)} className="flex items-center gap-1 hover:text-white transition-colors"><Settings className="w-3 h-3" /> Config</button>
                        <button onClick={resetConfig} className="flex items-center gap-1 hover:text-amber-400 transition-colors"><RotateCcw className="w-3 h-3" /> Reset</button>
                      </div>
                    </div>
                  </>
                )}
        ...
        <AnimatePresence>
          {showCloseModal && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm">
              <motion.div initial={{ scale: 0.9, y: 10 }} animate={{ scale: 1, y: 0 }} className="bg-[#0c0c0e] border border-white/10 rounded-2xl p-6 w-full max-w-xs shadow-2xl">
                ...
                <div className="space-y-2">
                  <button onClick={() => executeCloseAction("hide")} className="w-full bg-white/5 hover:bg-white/10 border border-white/10 text-white rounded-xl py-3 text-sm font-semibold transition-colors flex flex-col items-center">
                    <span>Minimize to Tray</span>
                    <span className="text-[10px] opacity-40 font-normal">VPN stays connected</span>
                  </button>
                  <button onClick={() => executeCloseAction("quit")} className="w-full bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 text-red-400 rounded-xl py-3 text-sm font-semibold transition-colors flex flex-col items-center">
                ...
                <button onClick={() => setShowCloseModal(false)} className="w-full mt-4 text-[11px] text-white/20 hover:text-white transition-colors">Cancel</button>
              </motion.div>
            </motion.div>
          )}
        </AnimatePresence>
